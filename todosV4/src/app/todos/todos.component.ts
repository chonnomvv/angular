...
@Component({
  ...
  template: `
    ...
    <div class="ver">Angular Version</div>

    <input class="input-todo" placeholder="What needs to be done?" autofocus (keyup.enter)="addTodo(input)" #input>
    <ul class="nav"(click)="tabMenu($event.target.id)" >
      <li id="all" [class.active]="status === 'all'">All</li>
      <li id="active" [class.active]="status === 'active'">Active</li>
      <li id="completed" [class.active]="status === 'completed'">Completed</li>
    </ul>

    <ul class="todos" >
      <li id="todo.id" class="todo-item" *ngFor="let todo of _todos;" (click)="toggleTodo(todo.id)">
        <input class="custom-checkbox" type="checkbox" id="todo.id" [checked]="todo.completed" >
        <label for="ck-myId">{{todo.content}}</label>
        <i class="remove-todo far fa-times-circle" (click)="remove(todo.id)"></i>
      </li>
    </ul>

    <div class="footer">
      <div class="complete-all" (click)="toggleAll()">
        <input class="custom-checkbox" type="checkbox" id="ck-complete-all">
        <label for="ck-complete-all">Mark all as complete</label>
      </div>
      <div class="clear-completed">
        <button class="btn" (click)="clearCompleted()" >Clear completed (<span class="completed-todos" >{{ClearCompletedCount}}</span>)</button>
...
export class TodosComponent {
  ...
  get _todos() {
    return this.todos.filter((todo) =>{
      if (this.status === 'all') return todo;
      else if(this.status === 'active') return !todo.completed;
      else if(this.status === 'completed') return todo.completed;
    })
  }
